...
```
3. Open `game/server/cbase.cpp`, then `#include "maphack_manager.h"`, and in function `CBaseEntityOutput::FireOutput()`, add this line of code at the end
```cpp
CMapHackManager::InvokeEntityOutputCallbacks( MapHackOutputCallbackParams_t( this, Value, pActivator, pCaller, fDelay ) );
```
4. Open `game/server/gameinterface.cpp`, then `#include "maphack_manager.h"`, and in function `CServerGameDLL::LevelInit()`, add this line after `g_flServerCurTime = gpGlobals->curtime;`
```cpp
